...
🔄 新增自动刷新课表流程
登录成功后，程序会先‌扫描页面状态‌，判断课表区域是否显示刷新按钮，再决定是否先刷新：
```bash
登录成功
  ↓
调用 GetStudentHome 接口获取 SJXS（系统进行中事项）
  ↓
判断：在线选课 / 重修申请 / 实验预约 是否有任意一项正在进行？
  ├─ 是 → 页面显示了「刷新」按钮 → 自动调用刷新流程：
  │       ① 获取校区/学期信息
  │       ② 重新请求最新课表（QueryStudentScheduleData）
  │       ③ 刷新实践课数据（QueryStudentPracticeData）
  │       → 用刷新后的数据进行课表比对
  └─ 否 → 页面无刷新按钮 → 直接获取课表正常比对
```
